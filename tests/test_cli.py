...
def test_parser_exposes_all_phase_commands():
    ...
    assert parser.parse_args(["knowledge", "install", "--url", "https://raw.githubusercontent.com/x/y/main/p.json", "--sha256", "0" * 64]).command == "knowledge"
...
def test_capture_persists_only_usage(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "stdin", type("Input", (), {"buffer": type("Buffer", (), {"read": lambda self, n: json.dumps(payload).encode()})()})())
    monkeypatch.setattr(sys, "argv", ["tokenomics", "capture", "--provider", "anthropic", "--model", "claude", "--path", str(tmp_path / "db")])
    main()
